让我们准备一个简单的示例来演示 Clang-Format 工具的实际操作。我们将创建一个名为 main.cpp 的基本 C++ 源文件，其中包含一些格式问题。然后，我们将在此文件上运行 Clang-Format 以自动更正格式并生成所做更改的报告：
...
如您所见，错误不是很具描述性。但是，大多数时候，开发人员可以了解代码出了什么问题。该工具不仅能够检测问题，还能修复问题。让我们运行该工具来修复格式问题 make clang-format 并查看结果：
...
代码现在已正确格式化，可以在项目中使用。此示例可以包含在您的章节中，以展示 Clang -Format 在真实场景中的实际应用。将来，开发人员可以向 .clang-format 文件添加更多格式化规则，并通过运行 make clang-format 命令重新格式化整个项目。
